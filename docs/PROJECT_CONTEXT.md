# Smart Canteen — robot-arm-service · Project Context

> Tài liệu nội bộ: kiến trúc, tiến độ, kế hoạch của service điều khiển robot.
...
- Repo này = **`robot-arm-service`** (Python/FastAPI): điều khiển **3 cánh tay FAIRINO FR3**
  ...
  (không cần C# gateway).
- Các thành phần anh em:
  - **Backend** .NET 8 + PostgreSQL — `TieHung23/SmartCanteen` (orders, wallet, sessions).
  - **Unity sim** (demo) — `TranMinh0903/FAIRINO_FR3_FoodServing` (Built-in RP).
  - **Unity URP shell** — `TranMinh0903/SmartCanteen-RobotSimulation`.
- **Tài liệu triển khai (v3.0):** `D:\FPTStudy\Capstone\KE_HOACH_TRIEN_KHAI_ROBOT_ARM.docx`
  (§13 kiến trúc Arm, §14 luồng phục vụ; 4 sơ đồ).
- Tham khảo pattern: `D:\FPTStudy\Capstone\xiangqi_robot_TrainningAI_Final_6`
  (FAIRINO FR5, Python, SDK in-process, teaching points, YOLO, DRY_RUN).

## 2. Kiến trúc chốt — PUSH / BUFFER

- **Trigger = THANH TOÁN** (không phải quét QR). Backend bind `ServingJob{Order↔Tray}` lúc trả
  tiền → **đẩy (SignalR)** cho service → robot **ráp sẵn** → khay xếp chờ trên **kệ pickup nhiều
  ô**. **QR pickup** của khách chỉ để **xác thực + lộ ô** khi tới lấy.
- **Robot tìm vị trí** = **teaching point** (tô ở đầu lane kệ con lăn; dạy 1 lần).
- **Nhận diện món** = **quét QR trên chén** (chống để nhầm lane); YOLO = lớp an toàn phụ.
...
- **3 loại mã (đừng lẫn):** QR chén = đúng MÓN · mã khay = khay là ORDER nào · QR pickup =
  khách xác thực.
...
- `app/signalr/client.py` — nối RA Backend, `on ReceiveJob` → submit; `ReportStatus`.
...
- **Motion thật** — gắp/đặt 3 độ cao an toàn (mở kẹp→SAFE_Z→PICK_Z→đóng→SAFE_Z→PLACE).
- **Teaching points** thật (`LANE_Sx_<dish>`, `PLACE_Sx`) — dạy trên controller.
...
- **KHÔNG hardcode** IP/secret trong code → để `.env`.
...
5. **Đóng gói** — `Dockerfile`, `pyproject.toml`, logging + retry, `docs/ARCHITECTURE.md`.
